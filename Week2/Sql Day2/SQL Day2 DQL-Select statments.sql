...
select Name,Location from tblEmployeeInfo 
 ...
 where location = 'Mumbai' or Location='Pune'
  ...
  insert tblemployeeinfo(employeeid,name,location,hiredate)
  values(1011,'Ram','null','1/1/2020')
  insert tblemployeeinfo(employeeid,name,location,hiredate)
  values(1012,'Sita','null','1/1/2020')
  insert tblemployeeinfo(employeeid,name,location,hiredate)
  values(1013,'Jamuna',null,'1/1/2020')
  select count(*),count(location) from tblemployeeInfo

   select Name,location from tblEmployeeInfo where Name like '%i'
...
-- Underscore(_) Search for single charater
 ...
 constraint pk_dept primary key,Deptname nvarchar(10) unique) 
   ...
   create table tblPerformance (Eid int references tblEmployeeInfo(Emp_id),
	...
	select DATEADD(day,60,GETDATE()) as [adding 30 with todays date]

	select *,DATEDIFF(d,leavestartdate,LeaveEndDate) as TotalNoOfLeaveDays 
	from tblEmployeeLeave

	select count(*) from tblEmployeeLeave
	where LeaveStartDate>='04/08/2020' and LeaveEndDate<='04/12/2020'
